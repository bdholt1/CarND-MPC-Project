...
double mph2mpsec(double x) { return x * 0.44704; }
...
Eigen::VectorXd apply_latency(Eigen::VectorXd& x0, double delta, double a, double dt) {
  ...
  // consider latency: set initial state to predicted state in 100ms
    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double v = mph2mpsec(speed);
          double delta = steering;
          double a = throttle;

          Eigen::VectorXd initial_state(4);
          initial_state << px, py, psi, v;
          // estimate what the new state would be in 100ms by applying the global kinematic model
          Eigen::VectorXd new_state = apply_latency(initial_state, delta, a, 0.1);
          double delayed_px = new_state(0);
          double delayed_py = new_state(1);
          double delayed_psi = new_state(2);
          double delayed_v = new_state(3);
